...
public class SellerControllerV3 {
    ...
    public ResponseEntity<?> addProduct(@RequestBody ProductVO productVO, @RequestHeader("Session-Id") String sessionId) throws NoSuchFieldException, IllegalAccessException {
        // 检查是否登录
        if (sessionId == null) {
    ...
    public ResponseEntity<?> updateProduct(@PathVariable String productId, @RequestBody ProductVO productVO, @RequestHeader("Session-Id") String sessionId) throws NoSuchFieldException, IllegalAccessException {
        if (sessionId == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
        }
        User user = userService.getUserInfo(sessionId);
    ...
    public ResponseEntity<?> deleteProduct(@PathVariable String productId, @RequestHeader("Session-Id") String sessionId) throws NoSuchFieldException, IllegalAccessException {
        if (sessionId == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
        }
        User user = userService.getUserInfo(sessionId);
